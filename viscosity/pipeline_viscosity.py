...
"""
Supervised VAE pipeline for viscosity prediction and COCONUT matching.

Modified version:
- SVAE trained on standardized data.
- Generated samples converted back to original values using inverse_transform.
- COCONUT matching performed in standardized descriptor space.
- Prediction models trained on standardized data.
"""
...
DEFAULTS = {
    "train_xlsx": "Viscosity_selected_features_balanced_original_values.xlsx",
    
    "train_smiles_xlsx": "Viscosity_selected_ind_descriptors____.xlsx",
    "coconut_xlsx": "coconut_selected_descriptors.xlsx",
    "outdir": "orchestrator_outputs_viscosity",
    "coconut_sheet": "viscosity",
}
...
def seed_everything(seed: int = 2026, deterministic: bool = True):
    """Seed Python, NumPy and PyTorch without disabling CPU parallelism."""
    seed = int(seed)
...
def make_torch_generator(seed: int):
    """Create a seeded CPU generator for reproducible DataLoader shuffling."""
...
def seed_worker(worker_id):
    """Seed each DataLoader worker reproducibly."""
...
POSSIBLE_TARGETS_MP = [
    "Viscosity", "Ln_Viscosity_cP", "ln_viscosity_cp",
    "Log_Viscosity", "log_viscosity", "Viscosity_cP",
    "viscosity", "ln_viscosity", "eta", "Eta",
]
...
def _find_mp_col(cols):
    ...
    for i, c in enumerate(low):
        if "viscos" in c or c in {"eta", "ln_eta", "log_eta"}:
            return cols[i]
...
def detect_columns(df: pd.DataFrame):
    ...
    if target is None:
        raise ValueError("Unable to detect the viscosity target column.")
...
def choose_coconut_sheet(coconut_xlsx: Path, preferred_sheet: str):
    ...
    if "viscosity" in sheets:
        print("[WARN] Requested sheet not found. Using 'viscosity'.")
        return "viscosity"
...
def step1_train_and_generate(
    ...
    joblib.dump(rf, outdir / "rf_full_viscosity.joblib")
    joblib.dump(gb, outdir / "gb_full_viscosity.joblib")
    ...
    gen_path = outdir / "generated_viscosity.xlsx"
    ...
    try:
        ...
        }).to_csv(outdir / "latent_pca_coords_training_viscosity.csv", index=False)

        pd.DataFrame({
            "PC1": coords_gen[:, 0],
            "PC2": coords_gen[:, 1],
            target_name: T_targets_K
        }).to_csv(outdir / "latent_pca_coords_generated_viscosity.csv", index=False)

        plt.figure(figsize=(6.5, 5.5))
        sc = plt.scatter(coords_train[:, 0], coords_train[:, 1], c=y.values, s=14, alpha=0.7, label="Training set")
        cbar = plt.colorbar(sc)
        cbar.set_label("ln(viscosity) (cP)")
        ...
        plt.tight_layout()
        plt.savefig(outdir / "latent_pca_combined_viscosity.png", dpi=160)
        plt.close()
...
def step2_match_to_coconut(
    ...
    if len(feature_cols) == 0:
        raise RuntimeError("No numeric descriptor detected in the generated file.")
    ...
    if coco.empty:
        raise RuntimeError(f"The sheet '{sheet}' is empty or no useful columns were read.")
    ...
    if len(pair_common) >= max(5, int(0.70 * len(feature_cols))):
        ...
        # then keep the n_keep best candidates according to the minimum distance.
        ...
        matched_path = outdir / "matched_real_from_coconut_viscosity.xlsx"
    ...
    if len(common1) < 3 or len(common2) < 3:
        raise RuntimeError(
            "Not enough common columns between generated and COCONUT.\n"
            f"Common comp1: {len(common1)} / {len(comp1_cols)}\n"
            f"Common comp2: {len(common2)} / {len(comp2_cols)}"
        )
    ...
    # Exclude SMILES pairs already present in the training set,
    # then keep the n_keep best pairs according to the total distance.
    out = filter_existing_training_pairs(
        out,
        train_xlsx=train_xlsx,
        train_smiles_xlsx=train_smiles_xlsx,
...
def step3_evaluate_models_train_real_validate_external(
    ...
    if len(common) == 0:
        raise RuntimeError("No common columns between real and external datasets.")
    ...
    for name, (estimator, dist) in models.items():
        ...
        plt.xlabel("Experimental ln(viscosity) (cP)")
        plt.ylabel("Predicted ln(viscosity) (cP)")
        plt.title(rf"COCONUT validation: $R^2$={metrics['Val_R2']:.3f}, RMSE={metrics['Val_RMSE']:.3f} cP")
        plt.legend(frameon=False)
        plt.tight_layout()

        fname = outdir / f"external_scatter_{name}_viscosity.png"
        plt.savefig(fname, dpi=160)
        plt.close()

        joblib.dump(best, outdir / f"best_{name}_viscosity.joblib")

    rep = pd.DataFrame(results).sort_values("Val_RMSE")
    rep_path = outdir / "external_eval_best_models_viscosity.csv"
    rep.to_csv(rep_path, index=False)

    joblib.dump(sX, outdir / "scaler_features_real_train_viscosity.pkl")
...
def load_training_with_pair_keys(
    ...
    """
    Charge le fichier de descripteurs utilisé pour le training et ajoute les paires SMILES
    depuis un second fichier aligné ligne par ligne.

    Cas prévu ici :
    - train_xlsx = Viscosity_selected_features_balanced_original_values.xlsx
      contient les descripteurs + Viscosity, mais pas les SMILES ;
    - train_smiles_xlsx = Viscosity_selected_ind_descriptors____.xlsx
      contient SMILES_1 / SMILES_2, dans le même ordre que train_xlsx.

    Si train_xlsx contains déjà les deux colonnes SMILES, train_smiles_xlsx est optionnel.
    """
    ...
    if c1_train is not None and c2_train is not None:
        ...
        if train_smiles_xlsx is None:
            raise RuntimeError(
                "The train_xlsx file does not contain SMILES pairs. "
        ...
        if len(train_df) != len(smiles_df):
            raise RuntimeError(
                ...
                f"train_smiles_xlsx contains {len(smiles_df)} lignes."
            )

        df = train_df.copy()
        # Only the SMILES columns are added to the descriptor file.
...
def keep_best_by_distance(df: pd.DataFrame, distance_col: str, n_keep=200):
    """Sort candidates by increasing distance and keep the n_keep best ones."""
    if n_keep is None:
        return df.reset_index(drop=True)

    if distance_col not in df.columns:
        print(f"[WARN] Distance column missing: {distance_col}. No n_keep filtering applied.")
        return df.reset_index(drop=True)
...
def filter_existing_training_pairs(
    ...
    """
    Remove rows whose pair (Smiles#1, Smiles#2) already exists
    ...
    if train_xlsx is None:
        print("[WARN] train_xlsx missing: existing-pair exclusion skipped.")
        return matched_df.reset_index(drop=True)
    ...
    print(f"[Step2] Pairs already present in the training set removed: {n_removed} / {n_before}")
...
def step4_check_existing_pairs(matched_xlsx: Path, train_xlsx: Path, outdir: Path, train_smiles_xlsx: Path = None):
    ...
    if c1_f1 is None or c2_f1 is None:
        print("[WARN] Pas de paires SMILES détectées dans matched. Step 4 skipped.")
        return None

    if c1_f2 is None or c2_f2 is None:
        print("[WARN] Pas de paires SMILES détectées dans train. Step 4 skipped.")
        return None
    ...
    out_xlsx = outdir / "viscosity_results_smiles_pairs.xlsx"
...
def make_masked_des_split(
    ...
    """
    Create a hidden-DES validation split by UNIQUE PAIR, even if the
    training file does not directly contain SMILES.

    Intended use:
    - train_xlsx: descriptor file used by the SVAE
      e.g. Viscosity_selected_features_balanced_original_values.xlsx
    - train_smiles_xlsx: file containing SMILES_1 / SMILES_2, in the same order
      e.g. Viscosity_selected_ind_descriptors____.xlsx

    All rows belonging to the same SMILES pair are assigned either to the training set
    ...
    """
    print("[HiddenValidation] Creating hidden-DES split by unique pairs")
    ...
    if stratify_target:
        try:
            ...
            print("[WARN] Target stratification skipped:", e)
            strat = None
    ...
    # These files contain the descriptors + SMILES_1 / SMILES_2 + pair_key.
...
def step_hidden_des_recovery_validation(
    ...
    """
    Validation 1: recovery of hidden DES.

    COCONUT matching is NOT used here.
    The generated candidates are directly compared with the hidden DES.
    For each hidden DES, the nearest generated candidate is identified
    ...
    if len(common) == 0:
        raise RuntimeError("No common column between masked training, hidden DES, and generated candidates.")
    ...
    for i in range(len(hidden_df)):
        ...
        row = {
            ...
            "absolute_error_ln_viscosity": abs(t_gen - t_real),
            "signed_error_ln_viscosity": t_gen - t_real,
        }
    ...
    global_summary = pd.DataFrame([{
        ...
        "R2": hidden_R2,
        "MAE_ln_viscosity": float(np.mean(np.abs(y_pred - y_true))),
        "RMSE_ln_viscosity": rmse(y_true, y_pred),
    ...
    for k in topk_values:
        ...
        top_rows.append({
            ...
            "R2": topk_R2,
            "MAE_ln_viscosity": float(np.mean(np.abs(yp - yt))),
            "RMSE_ln_viscosity": rmse(yt, yp),
        })
    ...
    # Figure: experimental values vs nearest generated values.
    try:
        plt.figure(figsize=(5.8, 5.8))
        ...
        plt.plot([lim_min, lim_max], [lim_min, lim_max], "--", linewidth=1.2)
        plt.xlabel("Experimental ln(viscosity) (cP)")
        plt.ylabel("Generated ln(viscosity) (cP)")
        plt.title(rf"Hidden-DES recovery: $R^2$={hidden_R2:.3f}, RMSE={rmse(y_true, y_pred):.3f}")
        plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="Supervised VAE for viscosity with hidden-DES validation and COCONUT matching"
    )
    ...
    parser.add_argument("--n_gen_coconut", type=int, default=None,
                        help="Number of generated candidates for the COCONUT phase. If omitted, uses --n_gen.")
    ...
    parser.add_argument("--seed", type=int, default=2026,
                        help="Random seed used for SVAE training and candidate generation.")
    parser.add_argument("--force_cpu", action="store_true",
                        help="Use CPU for maximum cross-run reproducibility.")
    parser.add_argument("--non_deterministic", action="store_true",
                        help="Allow faster non-deterministic CUDA operations.")
    ...
    parser.add_argument("--n_keep", type=int, default=200)
    ...
    summary_path = outdir / "pipeline_summary_viscosity_two_validations.json"
