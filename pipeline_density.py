...
"""
Supervised VAE pipeline for density prediction and COCONUT matching.
...
DEFAULTS = {
    "train_xlsx": "Density_selected_features_balanced_original_values.xlsx",
    # File aligned row by row with train_xlsx, containing SMILES_1 / SMILES_2.
    "train_smiles_xlsx": "Density_selected_ind_descriptors____.xlsx",
    "coconut_xlsx": "coconut_selected_descriptors.xlsx",
    "outdir": "orchestrator_outputs_density",
    "coconut_sheet": "density",
}
...
POSSIBLE_TARGETS_DENSITY = [
    "Density_g_per_cm3", "Density", "density",
    "density_g_per_cm3", "Density_g_cm3", "density_g_cm3",
    "Density (g/cm3)", "Density (g cm-3)",
    "rho", "Rho"
]


def _find_density_col(cols):
    cols = list(cols)
    low = [str(c).lower() for c in cols]

    for name in POSSIBLE_TARGETS_DENSITY:
        if name in cols:
            return name
...
def detect_columns(df: pd.DataFrame):
    target = _find_density_col(df.columns)

    if target is None:
        raise ValueError("Unable to detect the density target column.")
...
def choose_coconut_sheet(coconut_xlsx: Path, preferred_sheet: str):
    ...
    if "density" in sheets:
        print("[WARN] Requested sheet not found. Using 'density'.")
        return "density"
...
def step1_train_and_generate(
    ...
    joblib.dump(rf, outdir / "rf_full_density.joblib")
    joblib.dump(gb, outdir / "gb_full_density.joblib")
    ...
    gen_path = outdir / "generated_density.xlsx"
    ...
    try:
        ...
        }).to_csv(outdir / "latent_pca_coords_training_density.csv", index=False)

        pd.DataFrame({
            "PC1": coords_gen[:, 0],
            "PC2": coords_gen[:, 1],
            target_name: T_targets_K
        }).to_csv(outdir / "latent_pca_coords_generated_density.csv", index=False)
        ...
        plt.tight_layout()
        plt.savefig(outdir / "latent_pca_combined_density.png", dpi=160)
        plt.close()
...
def step2_match_to_coconut(
    ...
    target_col = _find_density_col(gen.columns)
    ...
    if len(pair_common) >= max(5, int(0.70 * len(feature_cols))):
        ...
        matched_path = outdir / "matched_real_from_coconut_density.xlsx"
...
def step3_evaluate_models_train_real_validate_external(
    ...
    for name, (estimator, dist) in models.items():
        ...
        plt.xlabel("Experimental density (g/cm³)")
        plt.ylabel("Predicted density (g/cm³)")
        plt.title(rf"COCONUT validation: $R^2$={metrics['Val_R2']:.3f}, RMSE={metrics['Val_RMSE']:.3f} g/cm³")
        plt.legend(frameon=False)
        plt.tight_layout()

        fname = outdir / f"external_scatter_{name}_density.png"
        plt.savefig(fname, dpi=160)
        plt.close()

        joblib.dump(best, outdir / f"best_{name}_density.joblib")

    rep = pd.DataFrame(results).sort_values("Val_RMSE")
    rep_path = outdir / "external_eval_best_models_density.csv"
    rep.to_csv(rep_path, index=False)

    joblib.dump(sX, outdir / "scaler_features_real_train_density.pkl")
...
def load_training_with_pair_keys(
    ...
    Cas prévu ici :
    - train_xlsx = Density_selected_features_balanced_original_values.xlsx
      contient les descripteurs + Density_K, mais pas les SMILES ;
    - train_smiles_xlsx = Density_selected_ind_descriptors____.xlsx
...
def step4_check_existing_pairs(matched_xlsx: Path, train_xlsx: Path, outdir: Path, train_smiles_xlsx: Path = None):
    ...
    out_xlsx = outdir / "density_results_smiles_pairs.xlsx"
...
def make_masked_des_split(
    ...
    - train_xlsx: descriptor file used by the SVAE
      e.g. Density_selected_features_balanced_original_values.xlsx
    - train_smiles_xlsx: file containing SMILES_1 / SMILES_2, in the same order
      e.g. Density_selected_ind_descriptors____.xlsx
...
def step_hidden_des_recovery_validation(
    ...
    for i in range(len(hidden_df)):
        ...
        row = {
            ...
            "absolute_error_g_per_cm3": abs(t_gen - t_real),
            "signed_error_g_per_cm3": t_gen - t_real,
        }
    ...
    global_summary = pd.DataFrame([{
        ...
        "MAE_g_per_cm3": float(np.mean(np.abs(y_pred - y_true))),
        "RMSE_g_per_cm3": rmse(y_true, y_pred),
        "R2": hidden_R2,
    ...
    for k in topk_values:
        ...
        top_rows.append({
            ...
            "MAE_g_per_cm3": float(np.mean(np.abs(yp - yt))),
            "RMSE_g_per_cm3": rmse(yt, yp),
            "R2": topk_R2,
        })
    ...
    try:
        ...
        plt.plot([lim_min, lim_max], [lim_min, lim_max], "--", linewidth=1.2)
        plt.xlabel("Experimental density (g/cm³)")
        plt.ylabel("Generated density (g/cm³)")
        plt.title("Hidden-DES recovery")
        plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="Supervised VAE for density with hidden-DES validation and COCONUT matching"
    )
    ...
    summary_path = outdir / "pipeline_summary_density_two_validations.json"
